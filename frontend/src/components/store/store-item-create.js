...
export default function StoreItemCreate() {
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append("opening_bid", openingBid);
    formData.append("contact", contact);
    formData.append("auction_end", auctionEnd);

    try {
      const response = await axios.post("/item/store/new/", formData, {
        headers: {
  ...
  useEffect(() => {
    if (localStorage.getItem("access_token")) {
      if (JSON.parse(localStorage.getItem("user_data")).type === "bidder") {
        setErrorMessage("Only sellers are allowed to create store items.");
      }
    } else {
