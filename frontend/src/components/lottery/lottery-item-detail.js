...
export default function LotteryItemDetail() {
  const { itemId } = useParams();
  const [lotteryItem, setLotteryItem] = useState(null);
  ...
  async function fetchLotteryItems() {
    await axios
      .get(`/item/lottery/${itemId}/`)
      .then((res) => {
        setLotteryItem(res.data);
        console.log(res.data);
        setBidAmount(res.data.item.bid);
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  async function fetchWishList() {
    await axios
      .get(`/item/lottery/${itemId}/wishlist/`)
      .then((res) => {
  ...
  const handleWishList = async () => {
    await axios
      .post(`/item/lottery/${itemId}/wishlist/`)
      .then((res) => {
        fetchWishList();
      })
      .catch((err) => {
          ...
          <div className="mx-auto">
            <div className="w-4/5 bg-white border border-gray-200 rounded-lg shadow">
              <img
                ...
                src={lotteryItem.item.image}
                alt={lotteryItem.item.title}
              />

              <div className="px-5 pb-5">
                <h5 className="mb-5 text-3xl font-semibold tracking-tight text-gray-900">
                  {lotteryItem.item.title}
                </h5>

                <div className="flex items-center justify-between">
                  <span className="text-xl font-bold text-gray-900">
                    $ {lotteryItem.item.bid}
                  </span>
                  ...
                  ) : lotteryItem.user_bid ? (
                    <div
                      type="button"
                      className="py-2.5 px-5 me-2 mb-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 opacity-50 cursor-not-allowed"
                    >
                      Bid Placed
                    </div>
                  ) : moment().isAfter(lotteryItem.item.auction_end) ? (
                    <div
                      type="button"
                      className="text-white bg-blue-700 focus:ring-4 focus:outline-none font-medium rounded-lg text-sm px-5 py-2.5 text-center opacity-50 cursor-not-allowed"
                    >
                ...
                <div className="mt-5">
                  <p className="text-sm text-gray-500">
                    {lotteryItem.item.description}
                  </p>

                  <span className="block mt-5 text-sm">
                    Auction Ends:{" "}
                    <strong>
                      {moment(lotteryItem.item.auction_end).format(
                  ...
                  <span className="block mt-5 text-sm">
                    Seller: <strong>{lotteryItem.item.seller}</strong>
                  </span>

                  <span className="block mt-5 text-sm">
                    Contact: <strong>{lotteryItem.item.contact}</strong>
                  </span>
                </div>
              </div>
            </div>
          ...
          <div>
            ...
            {userData.type === "bidder" ? (
              ...
              <div className="bg-white border border-gray-200 rounded-lg shadow p-8">
                ...
                ) : moment(lotteryItem.item.auction_end).isBefore(
                    new Date(),
                  ) ? (
                  <button
            ...
            <div className="relative overflow-x-auto">
              <table className="w-full text-sm text-left rtl:text-right text-gray-500">
                ...
                <tbody>
                  {lotteryItem.bids.length > 0 ? (
                    lotteryItem.bids.map((bid) => (
